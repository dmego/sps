...
Page({
  ...
  onShow: function () {
    ...
    car.get(carId).then(res => {
      ...
      if (carNumber.length == 7) {
        that.setData({
          inputPlates: {
            index0: carNumber.charAt(0),
            index1: carNumber.charAt(1),
            index2: carNumber.charAt(2),
            index3: carNumber.charAt(3),
            index4: carNumber.charAt(4),
            index5: carNumber.charAt(5),
            index6: carNumber.charAt(6),
            index7: ""
          }
        })
      } else {
  ...
  submitButtonClick: function () {
    ...
    car.find().then(res => {
      for (let item of res) {
        if (item.carNumber == n && item.carNumber != carNumber) {
          itemCar = item;
          bool = true;
          break;
        }
